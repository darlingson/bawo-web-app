...
export function makeMove(gameState: GameState, row: number, pitIndex: number): GameState {
...
export function checkGameOver(gameState: GameState): boolean {
  ...
  const noValidMovesForPlayer1 = gameState.board[1].every(pit => pit.seeds === 0);
  const noValidMovesForPlayer2 = gameState.board[2].every(pit => pit.seeds === 0);
